...
static void LmhpRemoteMcastSetupOnMcpsIndication( McpsIndication_t *mcpsIndication )
{
    ...
    if( dataBufferIndex != 0 )
    {
        ...
        DBG( "McAddr      : %08lX\n", (unsigned long) McSessionData[0].McGroupData.McAddr );
        ...
        DBG( "\n" );
        DBG( "McFCountMin : %lu\n",  (unsigned long) McSessionData[0].McGroupData.McFCountMin );
        DBG( "McFCountMax : %lu\n",  (unsigned long) McSessionData[0].McGroupData.McFCountMax );
        DBG( "SessionTime : %lu\n",  (unsigned long) McSessionData[0].SessionTime );
        DBG( "SessionTimeT: %d\n",  McSessionData[0].SessionTimeout );
        if( McSessionData[0].RxParams.Class == CLASS_B )
        {
            DBG( "Rx Freq     : %lu\n", (unsigned long) McSessionData[0].RxParams.Params.ClassB.Frequency );
            DBG( "Rx DR       : DR_%d\n", McSessionData[0].RxParams.Params.ClassB.Datarate );
            DBG( "Periodicity : %u\n", McSessionData[0].RxParams.Params.ClassB.Periodicity );
        }
        else
        {
            DBG( "Rx Freq     : %lu\n", (unsigned long) McSessionData[0].RxParams.Params.ClassC.Frequency );
